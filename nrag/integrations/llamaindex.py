...
def to_llamaindex_retriever(nrag, *, k=None):
    """Return a LlamaIndex ``BaseRetriever`` backed by ``nrag.search``.

    Usage::

        from nrag.integrations import to_llamaindex_retriever
    ...
    class NragRetriever(BaseRetriever):
        """LlamaIndex retriever that delegates to a NRAG index (no embeddings, no vector DB)."""
